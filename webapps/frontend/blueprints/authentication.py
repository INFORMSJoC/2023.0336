...
_APP_URL = _CONFIG_INI.get("FRONTEND", "URL")
...
def create_session_data(account, rememberme=False, respond_success=True):
	...
	resp.set_cookie('jwt', json_web_token(account['id'], app.secret_key))
	if rememberme:
...
def login():
	...
	if request.method == 'POST' and 'username' in request.form and 'password' in request.form and 'token' in request.form:
		...
		if account:
			...
			cursor.execute('DELETE FROM user_login_attempts WHERE ip_address = %s', (request.environ['REMOTE_ADDR'],))
			mysql.connection.commit()
...
def login_attempts(update = True):
    ...
    if update:
        cursor.execute('INSERT INTO user_login_attempts (ip_address, `date`) VALUES (%s,%s) ON DUPLICATE KEY UPDATE attempts_left = attempts_left - 1, `date` = VALUES(`date`)', (ip, str(now),))
        mysql.connection.commit()
	# Retrieve the login attemmpts
    cursor.execute('SELECT * FROM user_login_attempts WHERE ip_address = %s', (ip,))
    ...
    if login_attempts:
        ...
        if datetime.datetime.strptime(now, '%Y-%m-%d %H:%M:%S') > expire:
            # Delete the entry
            cursor.execute('DELETE FROM user_login_attempts WHERE id_address = %s', (ip, session['id'],))
            mysql.connection.commit()
            login_attempts = []
...
def admin_settings(msg):
	...
	if request.method == 'POST' and request.form:
		...
		for key, value in data.items():
			...
			# Update setting
			cursor.execute('UPDATE user_settings SET setting_value = %s WHERE setting_key = %s', (value,key,))
			mysql.connection.commit()
...
def get_settings():
	cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
	cursor.execute('SELECT * FROM user_settings ORDER BY id')
