...
class Grid(object):
    ...
    def update_current_conditions(self, timestep, weather):
        ...
        for type in self._generators:
            for generator in self._generators[type]:
                generator.update_current_conditions(timestep.time_period().end(), weather)
    ...
    def _min_available_energy(self, type, duration, fully_online=False):
        ...
        diesel_consumption = diesel_level - self._diesel_level
    ...
    def _control_logic_s(self, previous_case, powers):
        """Control logic: favor renewables, then diesel, then BESS"""
    ...
    def operate(self, previous_case, load, duration, control):
        """Generate power from all generating components over an input duration
        control        string name of control logic function to use
        ...
        case = 5       load cannot be met with all generators and BESS discharges
        ...
        fully_online_power=copy.deepcopy(self._available_power)
        self._available_power = {}
