...
def get_single(component_id, objectFlag=False):
    ...
    try:
        component_info = mysql_helpers.DB.query(
        ...
        raise mysql_helpers.MicrogridDBException("component_get failed for component and type " \
                + "for component_id="+str(component_id)+"\n"+str(error))
    try:
        attributes_list = mysql_helpers.DB.query(
        ...
        raise mysql_helpers.MicrogridDBException("component_get failed for spec meta and data for component_id=" \
            +str(component_id)+"\n"+str(error))
    if objectFlag:
...
def get_all(user_id):
    ...
    try:
        component_ids = mysql_helpers.DB.query(
        ...
        raise mysql_helpers.MicrogridDBException("components_get failed for user_id=" \
            +str(user_id)+"\n"+str(error))
    components = []
...
def update_attributes(component_id, component_specs):
    ...
    for meta_id, value in component_specs.items():
        try:
            min_max = mysql_helpers.DB.query(
                """SELECT minVal, maxVal
                    FROM component_spec_meta
                    WHERE id = %s""",
            [meta_id], output_format="dict")[0]
        except Exception as error:
            raise mysql_helpers.MicrogridDBException("component_update_attributes failed to get min/max for meta id = {0}\n{1}".format(
                meta_id, error))
        ...
        try:
            mysql_helpers.validate_input_value("float", value, min_value, max_value, "for meta id = {0} of ".format(meta_id))
        except Exception as error:
            remove(component_id)
        ...
        try:
            ...
            mysql_helpers.DB.insert_update("component_spec_data", component_spec_data)
        except Exception as error:
            raise mysql_helpers.MicrogridDBException("component_update_attributes failed update for spec_data = " \
                +component_spec_data+"\n"+str(error))
    return True, "Success"
...
def add(user_id, type_id:int, name, description, specifications):
    ...
    if not mysql_helpers.unused_name_in_table(user_id, "component", name):
        return False, "Component {0} already exists for this user".format(name)
    try:
        component_id = mysql_helpers.DB.insert(
            table_name="component", 
            data_dict={ "name": name, "description":description, "componentTypeId": type_id, "id": None }
        )
    except Exception as error:
        raise mysql_helpers.MicrogridDBException("component_add insert failed for component with name = " \
            +name+" and type id = "+str(type_id)+"\n"+str(error))
    try:
        mysql_helpers.DB.insert(
            table_name="component_user", 
            data_dict={ "componentId":component_id, "userId":user_id, "permissionId": mysql_helpers.PERMISSION_WRITE }
        )
    except Exception as error:
        raise mysql_helpers.MicrogridDBException("component_add insert failed for component_user with component id = " \
            +str(component_id)+" and user id = "+str(user_id)+"\n"+str(error))
    try:
        component_defaults = mysql_helpers.DB.query( ### CAN BE CACHED TO IMPROVE EFFICIENCY
            """SELECT id, value 
                FROM component_spec_meta
                WHERE componentTypeId = %s AND value IS NOT NULL""", 
            values=[type_id])
    except Exception as error:
        raise mysql_helpers.MicrogridDBException("component_add select failed for spec_meta with component type id = " \
            +str(type_id)+"\n"+str(error))
...
def remove(component_id):
    ...
    try:
        mysql_helpers.DB.delete("component", {"id": component_id})
    except Exception as error:
        raise mysql_helpers.MicrogridDBException("component_remove failed for component id = "+str(component_id)+"\n"+str(error))
...
def types_get():
    ...
    try:
        component_types_list = mysql_helpers.DB.query("SELECT * FROM component_type ORDER BY displayOrder")
    except Exception as error:
        raise mysql_helpers.MicrogridDBException("component_types_get select failed\n"+str(error))
    try:
        component_spec_meta = mysql_helpers.DB.query(
            """SELECT *
            FROM component_spec_meta
            ORDER BY displayOrder""", output_format="dict")
    except Exception as error:
        raise mysql_helpers.MicrogridDBException("component_types_get failed for select component_spec_meta\n"+str(error))
